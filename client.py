...
""" To read from the file on byte by byte basis. offset_num to indicate from where to start reading the file """
...
def checksum(msg):
    if (len(msg) % 2) != 0:
        msg += "0".encode('utf-8')              # For odd and even number

    s = 0
    for i in range(0, len(msg), 2):             # 2 words
        w = msg[i] + ((msg[i+1]) << 8)
        s = add_carry(s, w)
    return ~s & 0xffff
...
rem_size = size_file
segment_num = 0                         # 0 and 1 for stop and wait
offset_num = 0
start = datetime.datetime.now()

while(rem_size>=0):
    list_ack = list(server_ip_address)  # Reference copy of ip addresses
    ack_count = 0
    buffer_temp = b''                   # To store the file ( Entire file )
    print('Remaining size:', rem_size)

    while(len(buffer_temp) <= MSS):
        temp = rdt_send(offset_num)

        if((len(temp)==1) and (len(buffer_temp)<MSS)):  # To add till the MSS
            ...
            for t in thread_list:
                t.join()                        # Waits till the execution of all the threads

            while(ack_count != len(server_ip_address)):
                client_sock.settimeout(0.05)                    # Single timeout for all receivers
                try:
                    ack, address = client_sock.recvfrom(MSS)       # Getting ack from server
                    server_seq_num = int(ack[0:32], 2)
                    if server_seq_num == segment_num:               # Verifying sequence number is same
                        if address[0] in list_ack:                          # Address and Port Number
                            ack_count += 1
                            list_ack.remove(address[0])                     # Remove address after getting ack from List

                except socket.timeout:
                    print('Timeout, sequence number = ', segment_num)
                    for ip_address in list_ack:                      # Retransmitting data for server not received file
                        server_address = (ip_address, server_port)
                        new_thread = serverThread(data, server_address)
                        new_thread.start()
                        thread_list.append(new_thread)
        ...
        if((len(temp)==1) and (len(buffer_temp) == MSS)):
            segment_num_bin = '{0:032b}'.format(segment_num)            # Append all the headers in binary format
            check = checksum(buffer_temp)                               # Checksum performed for data
    ...
    if segment_num == 1:   # For the following segments after successful txn of first segment
        segment_num = 0
    else:
        segment_num += 1
